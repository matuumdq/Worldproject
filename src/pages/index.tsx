...
const IndexRoute: React.FC<Props> = ({products}) => {
    function renderModal(prod) {
        const msg = `https://api.whatsapp.com/send?phone=+5492236681270&text=Hola,%20estoy%20interesado%20en%20el%20producto:%20${prod.title},%20deseo%20mas%20info`;
        return (
        ...
        {prod.map((item) =>
            item.id === '' || item.title === '' ? null : (
                <div key={item.id} className='border border-slate-600 shadow-sm hover:shadow-none hover:border-green-400 rounded-md hover:scale-105 duration-300 flex flex-col items-center bg-black'>
                 <img as={motion.img} layoutId={item.image} src={item.image} alt={item.title} className='w-full h-[200px] object-scale-down bg-white rounded-t-md' />   
            ...
            <AnimatePresence>
                {selectedProd && 
                    <motion.div  
                        ...
                        className='h-auto min-h-screen w-full bg-black/90 fixed top-0 left-0 flex flex-col item justify-center transition-all ease-in duration-300 md:px-10 lg:px-20 overflow-y-auto z-50'
                    >
